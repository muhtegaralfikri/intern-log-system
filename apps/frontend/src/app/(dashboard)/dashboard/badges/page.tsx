...
export default function BadgesPage() {
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <p className="text-gray-600">
            <span className="font-semibold text-green-600">{myBadges.length}</span> dari{' '}
      ...
      {newBadges.length > 0 && (
        <Card className="mb-6 bg-green-50 border-green-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-green-800">Selamat! Anda mendapatkan badge baru:</p>
              <p className="text-green-700">{newBadges.join(', ')}</p>
            </div>
          </div>
        </Card>
      )}
      ...
      <div className="mb-8">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Badge yang Diperoleh</h3>
        {myBadges.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {myBadges.map((badge) => (
              <Card key={badge.id} className="text-center py-6 bg-gradient-to-b from-yellow-50 to-white border-yellow-200">
      ...
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Semua Badge</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {allBadges.map((badge) => {
            ...
            return (
              <Card
                key={badge.id}
                className={`text-center py-6 ${isEarned ? 'bg-white' : 'bg-gray-100 opacity-60'}`}
              >
